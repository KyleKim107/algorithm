...
* Integer
  ...
  * int: 4 byte
    ...
    * 범위: -2^31 ~ 2^31 -1 (+- 21억)(-2147483647~2147483647)
...
* Throw 예약어
  ...
  * 예외가 발생한 메서드를 호출한 지점으로 예외를 전달하여 처리하는 방법
  ...
  ![에러가 발생한 메서드에서 메인 메서드로 전달함](images/20210706_191654.png)

  * 이런 경우는 메인 메서드에도 try - catch문을 넣어 놔야한다

  * 메인메서드로 예외를 보낼때, runtime excpetion들의 자식일 경우 throw구문들 사용하지 않아도 throw됨

> 사용자 예외 클래스

* API에서 제공되지 않는 예외를 커스텀함

  * 사용자 정의예외 클래스를 발생기켜 처리하려면, 반드시 throw예약어를 이용해야함

![runtime exception을 상속 하면, 18번 라인의 throw를 할 필요 없어진다](images/20210706_193021.png)

### Java.lang

* 자바 프로그램을 구현할때 가장 기본이 되는 필수 클래스와 인터페이스들이 포함된 패키지

* 모든 자바 클래스들은 Object클래스로 부터 상속받는다는 의미임

  * 자동상속 받음, 자바 API의 모든 클래스와 사용자가 정의한 모든 클래스의 최상위 클래스

* java.lang 가장 기본이 되는 필수 클래스들과 인터페이스들이 포함된 패키지

* 자바랭 패키지 클래스드들은 import필요 없어(한다고 에러뜨는건 아님)

* == 연산자 대신 object들은 equals()를 쓴다

  * 참조형은 값이 메모리 주소값이라 그렇다

  * 객체의 내용을 사용함

![equal](images/20210706_205536.png)

> ToString()

* 수시로 변하는 객체의 변수값들을 화면에 간단히 출력 하고자 할떄

  * 그냥 객체를 프린트 해도 To.String과 같은 포멧으로 나옴

![toString](images/20210706_211658.png)

> Wrapper클래스 소개

![wrapper](images/20210706_212717.png)

> 문자열

![스트링 클래스](images/20210706_215042.png)

* 스트링 객체는 한번 생성되면 변경 불가능함

* concat() : +하기

![같은 문자열 객체는 같은 주소 공유](images/20210706_215904.png)

![같은 문자열 객체는 같은 주소 공유2](images/20210706_220041.png)

![스트링 클래스1](images/20210706_220738.png)

![스트링 클래스2](images/20210706_220707.png)

> StringBuffer 클래스

* 자바 프로그램 내에서 변하는 문자열을 다를때 사용함

* 스트링 버퍼 클래스는 크기가 동적임

* Default값이 16개 문자 저장 가능

![string buffer](images/20210706_221231.png)

![string buffer](images/20210706_221334.png)

![buffer](images/20210706_221357.png)

* StringBuffer는, '+'연산자를 사용해서 문자열을 결합할 수 있는가

  * NO!!!

* new 연산자 없이 객체 생성 가능? ㄴㄴ

![str](images/20210706_222608.png)

![str vs buffer](images/20210706_222844.png)
...
![float](images/20210719_141706.png)

> 전위 후의

![a](images/20210719_141842.png)

## 객체 생성과 메모리

![a](images/20210720_152100.png)

* 클래스로딩시점

  * Node01 head = new Node01(); 같은

  * 클래스 변수는 클래스 로딩시 메모리 등록

  * 소멸시점: 프로그램 종료시

* 명령어는 무조건 메소드의 블록 안에 있어야 한다

* 네이밍 룰로 알아보는 클래스 /메소드/변수/ 상수
  ...
  * 리턴타입
    ...
    * void와 return을 함께 사용 가능

  * Variable Argument

    * 메소드의 가장 마지막에 배치한다

![a](images/20210720_173032.png)

* 정답순 : b / c/ e/ d/ f /a

![a](images/20210720_175150.png)

![a](images/20210720_175730.png)
